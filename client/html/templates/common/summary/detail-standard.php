		...
		<?php foreach( $this->summaryBasket->getProducts() as $position => $product ) : $totalQuantity += $product->getQuantity(); ?>
			<tr class="product <?= ( isset( $errors['product'][$position] ) ? 'error' : '' ); ?>">
				...
				<td class="details">
					...
					?>
					<a class="product-name" ><?= $enc->html( $product->getName(), $enc::TRUST ); ?></a>
					<p class="code">
...
<div class= "checkout-table" style="display:none">
<?php $totalQuantity = 0; ?>
<div class="your-order-box gray-bg  " >
                            <div class="your-order-product-info">
								...
								?>
										...
										<a  class="your-order-middle-left font--semi-bold">
...
<div class="your-order-box gray-bg  " >
				...
				<?php endif; ?>
								...
								?>
										...
										<a  class="your-order-middle-left font--semi-bold">
										<?= $enc->html( $product->getName(), $enc::TRUST ); ?> 	</a>
								...
								<span class="your-order-middle-right font--semi-bold"><?= $enc->html( sprintf( $priceFormat, $this->number( $product->getPrice()->getValue() * $product->getQuantity(), $precision ), $priceCurrency ) ); ?></span>
									...
									<div class="quantity">	
									<a><?= $enc->html( $this->translate( 'client', 'Quantity' ), $enc::TRUST ); ?></a>
									<?php if( $modify && ( $product->getFlags() & \Aimeos\MShop\Order\Item\Base\Product\Base::FLAG_IMMUTABLE ) == 0 ) : ?>
						<?php if( $product->getQuantity() > 1 ) : ?>
							<?php $basketParams = array( 'b_action' => 'edit', 'b_position' => $position, 'b_quantity' => $product->getQuantity() - 1 ); ?>
							<a class="minibutton change" href="<?= $enc->attr( $this->url( $basketTarget, $basketController, $basketAction, $basketParams, [], $basketConfig ) ); ?>">−</a>
					...
					<?php endif; ?></div>
								...
								<div class="your-order-bottom d-flex justify-content-between">
									<h6 class="your-order-bottom-left font--bold"><?= $enc->html( $this->translate( 'client', 'Sub-total' ) ); ?></h6>
									<?php if( $this->summaryBasket->getPrice()->getCosts() > 0 ) : ?>
		
										<span class="your-order-bottom-right font--semi-bold"><?= $enc->html( sprintf( $priceFormat, $this->number( $this->summaryBasket->getPrice()->getValue(), $precision ), $priceCurrency ) ); ?></span>
								...
								<div class="your-order-bottom d-flex justify-content-between">
									<h6 class="your-order-bottom-left font--bold"><?= $enc->html( $this->translate( 'client', 'Payment costs' ) ); ?></h6>
									<?php if( ( $costs = $this->get( 'summaryCostsPayment', 0 ) ) > 0 ) : ?>
								...
								<?php if( $priceTaxflag === true ) : ?>
								<div class="your-order-bottom d-flex justify-content-between">
									<h6 class="your-order-bottom-left font--bold"><?= $enc->html( sprintf( $this->translate( 'client', '%1$d article', '%1$d articles', $totalQuantity ), $totalQuantity ) ); ?></h6>
									<?php if( ( $costs = $this->get( 'summaryCostsPayment', 0 ) ) > 0 ) : ?>
		
										<span class="your-order-bottom-right font--semi-bold"><?= $enc->html( sprintf( $priceFormat, $this->number( $this->summaryBasket->getPrice()->getValue() + $this->summaryBasket->getPrice()->getCosts(), $precision ), $priceCurrency ) ); ?></span>
								...
								<?php if( $this->summaryBasket->getPrice()->getRebate() > 0 ) : ?>
								<div class="your-order-bottom d-flex justify-content-between">
									<h6 class="your-order-bottom-left font--bold"><?= $enc->html( $this->translate( 'client', 'Included rebates' ) ); ?></h6>
									<?php if( ( $costs = $this->get( 'summaryCostsPayment', 0 ) ) > 0 ) : ?>
		
										<span class="your-order-bottom-right font--semi-bold"><?= $enc->html( sprintf( $priceFormat, $this->number( $this->summaryBasket->getPrice()->getRebate(), $precision ), $priceCurrency ) ); ?></span>
...
<?php /* <div class="summary-mobile-table" style="display:none">
	<div class="details">
		<?php $totalQuantity = 0;?>
		...
		<table class="mobile-tfoot">
		<?php if( $this->summaryBasket->getPrice()->getCosts() > 0 ) : ?>
			<tr class="subtotal">
				<td class=""><?= $enc->html( $this->translate( 'client', 'Sub-total' ) ); ?></td>
				<td class="price"><?= $enc->html( sprintf( $priceFormat, $this->number( $this->summaryBasket->getPrice()->getValue(), $precision ), $priceCurrency ) ); ?></td>
				<?php if( $modify ) : ?>
					<td class="action"></td>
				<?php endif; ?>
